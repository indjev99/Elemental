...
\node (title) at (0.5 * \cardwidth, \cardheight - \cardroundingradius)
    ...
    text width = \cardwidth - 2 * (\cardroundingradius + 1mm),
...
\node (text) at (0.5 * \cardwidth, \cardroundingradius + \tvheight + \tvspace)
    ...
    text width = \cardwidth - 2 * (\cardroundingradius + 1mm),
    text depth = \cardtextboxheight,
    align = justify]
    { #3 };

\node (value)
    [ fill = \cardvaluecolour!20, draw = \cardvaluecolour, thick,
    below right = \tvspace + 0.5 * \tvheight and 0mm of text, anchor = east,
    minimum height = \tvheight]
    { #4 Gold };

\node (type)
    [ fill = \cardtypecolour!20, draw = \cardtypecolour, thick,
    below left = \tvspace + 0.5 * \tvheight and 0mm of text, anchor = west,
    minimum height = \tvheight]
    { #2 };
